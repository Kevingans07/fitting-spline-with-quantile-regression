...
lines(x, X %*% fit_pen_2$par, col = "brown", lwd = 2)

lines(x, sin_quantile_90, col = "blue", lty = 2, lwd = 5) # True 90th quantile
legend("topright", legend = c("Data", "Fitted", "True 90th quantile"), 
...
sigma <- 1           # fixed
#Define lambda
lambda_grid <- 10^seq(-3, 25, length.out = 20)  # grid of lambdas to try (10^-3 to 10^25)
set.seed(42)
...
if (length(start_full) != ncol(X)) start_full <- rep(0, ncol(X))
 ...
 # Fit final model
  ...
  lines(dat$x, as.numeric(X %*% fit_pen_1$par), col = "brown", lwd = 2)
  lines(x, sin_quantile_90, col = "blue", lty = 2, lwd = 5) # True function
  
  legend("topright",
       legend = c(
         paste("for tau =", tau," | Optimal λ=", round(best_lambda, 4)),
         paste("λ = ", lambda_1),
         "True function = 90th quantile"
       ),
