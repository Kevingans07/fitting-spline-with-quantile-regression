...
for (i in seq_along(lambda_grid)){
  ...
  for (fold in 1:k_folds) {
      ...
      beta_hat <- fit_pen_2$par
      
      #compute validation pinball loss, without penalty from pen_ssqr
      validation_losses[fold] <- sum((tau - (y_val - X_val %*% beta_hat < 0)) 
                                     * (y_val - X_val %*% beta_hat))
 ...
 # Fit final model
  fit_final_with_best_lambda <- optim(par = start_coef,
                            fn = pen_ssqr,
                            y = y_train,
                            X = X_train,
                            tau = tau,
  ...
  plot(x, y, col = "grey")
  lines(x, X %*% fit_final_with_best_lambda$par, col = "red", lwd = 2)
