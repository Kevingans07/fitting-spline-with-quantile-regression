...
# Plot 1: Original single seed version
plot(x, y_sin_gamma, col = "grey", pch = 16, cex = 0.4,
     main = "Original (Single Seed)")
lines(x, X %*% fits[["0.1"]], col = "blue", lwd = 2)
lines(x, X %*% fits[["0.5"]], col = "black", lwd = 2)
lines(x, X %*% fits[["0.9"]], col = "red", lwd = 2)
legend("topright", legend = c("10%", "50%", "90%"),
       col = c("blue", "black", "red"), lwd = 2)


# Plot 2: All 100 simulations overlaid
plot(x, y_sin_gamma, col = "grey", pch = 16, cex = 0.4,
...
# Plot 3: Mean ± SD bands across simulations
...
plot(x, true_q50, type = "n", ylim = range(y_sin_gamma),
     main = "Mean ± SD Across 100 Simulations",
     xlab = "x", ylab = "y")
...
# Plot 4: Comparison of original vs mean of simulations
plot(x, y_sin_gamma, col = "grey", pch = 16, cex = 0.4,
     main = "Original vs Mean of 100 Sims")
...
for (i in seq_along(taus)) {
  ...
  # Original (dashed)
  lines(x, X %*% fits_original[[as.character(tau)]], 
        col = col_val, lwd = 2, lty = 2)
}

legend("topright",
       legend = c("Mean of 100", "Original seed=42"),
       lty = c(1, 2), lwd = 2)
...
for (tau in taus) {
  ...
  sd_fit <- apply(all_fits[, , as.character(tau)], 2, sd)
  fitted_sd <- X %*% sd_fit
  cat("Mean pointwise SD:", mean(fitted_sd), "\n")
  
  # Distance from original
  mean_fit <- apply(all_fits[, , as.character(tau)], 2, mean)
  dist_from_original <- sqrt(mean((X %*% mean_fit - X %*% fits_original[[as.character(tau)]])^2))
  cat("RMSE from original:", dist_from_original, "\n")
}
